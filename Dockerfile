...
CMD ["bash", "-c", "./pulseaudio-setup.sh && Xvfb :1 -screen 0 768x768x24 & sleep 10 && DISPLAY=:1 ./nesexe \"./rom/${GAME}.nes\" & sleep 10 && ffmpeg -hwaccel cuda -f pulse -i default -f x11grab -s 768x768 -i :1 -map 0:a:0 -c:a libopus -compression_level 0 -b:a 24k -af aresample=async=0.1 -map 1:v:0 -r 75 -c:v h264_nvenc -preset p1 -tune ll -b:v 1000k -f rtsp rtsp://localhost:8554/mystream"]
EXPOSE 8080
